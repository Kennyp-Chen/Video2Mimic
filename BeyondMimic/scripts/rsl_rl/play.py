...
parser.add_argument(
    ...
    type=str,
    default=None,
    help="Local path to the model checkpoint (e.g., logs/rsl_rl/exp_name/model_1000.pt). Alternative to wandb_path.",
)
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlOnPolicyRunnerCfg):
    ...
    if args_cli.local_path:
        # Load from local path
        resume_path = args_cli.local_path
        if not os.path.isabs(resume_path):
            resume_path = os.path.abspath(resume_path)
        
        if not os.path.exists(resume_path):
            raise FileNotFoundError(f"Local model checkpoint not found: {resume_path}")
        
        print(f"[INFO]: Loading model checkpoint from local path: {resume_path}")
        
        # Motion file only from CLI argument
        if args_cli.motion_file is not None:
            print(f"[INFO]: Using motion file from CLI: {args_cli.motion_file}")
            env_cfg.commands.motion.motion_file = args_cli.motion_file
        else:
            print("[WARN]: No motion file specified. Using default from config.")
        ...
        # Load from default log directory
    ...
    #     normalizer = None
    #     # 先检查新版结构
    #     if hasattr(ppo_runner, 'alg') and hasattr(ppo_runner.alg, 'policy'):
    #         # 检查策略中是否有归一化器
    ...
    export_motion_policy_as_onnx(
        ...
        # normalizer=get_normalizer(ppo_runner),
