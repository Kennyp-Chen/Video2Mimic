...
def export_motion_policy_as_jit(
    ...
    """Export motion policy into a Torch JIT file.

    Args:
        env: The Isaac Lab environment.
        actor_critic: The policy torch module.
        normalizer: The empirical normalizer module. If None, Identity is used.
        path: The path to the saving directory.
        filename: The name of exported JIT file. Defaults to "policy.pt".
    """
    policy_exporter = _TorchMotionPolicyExporter(env, actor_critic, normalizer)
    policy_exporter.export(path, filename)
...
class _TorchMotionPolicyExporter(_TorchPolicyExporter):
    """Exporter of motion policy into JIT file."""
    ...
    def __init__(self, env: ManagerBasedRLEnv, actor_critic, normalizer=None):
        ...
        self.body_ang_vel_w = cmd.motion.body_ang_vel_w.to("cpu")
        self.time_step_total = self.joint_pos.shape[0]
        # Override forward method for recurrent networks to include time_step
        if self.is_recurrent:
            self.forward = self.forward_lstm

    def forward(self, x, time_step):
        """Forward pass that returns actions and motion data.

        Args:
            x: Observation tensor.
            time_step: Time step tensor.

        Returns:
            Tuple containing actions and motion data (joint_pos, joint_vel, body_pos_w, body_quat_w, body_lin_vel_w, body_ang_vel_w).
        """
        time_step_clamped = torch.clamp(time_step.long().squeeze(-1), max=self.time_step_total - 1)
    ...
    def forward_lstm(self, x, time_step):
        """Forward pass for recurrent networks that returns actions and motion data.
    ...
    def export(self, path, filename):
        """Export the policy to a JIT file.

        Args:
            path: The path to the saving directory.
            filename: The name of exported JIT file.
        """
        os.makedirs(path, exist_ok=True)
        path = os.path.join(path, filename)
        self.to("cpu")
        traced_script_module = torch.jit.script(self)
        traced_script_module.save(path)  # type: ignore
