...
def load_smplx_file(smplx_file, smplx_body_model_path):
    ...
    # Handle gender which might be numpy array or string
    gender_data = smplx_data["gender"]
    if hasattr(gender_data, 'item'):
        gender_str = str(gender_data.item())
    else:
        gender_str = str(gender_data)
    
    print(f"DEBUG: smplx_body_model_path = {smplx_body_model_path}")
    print(f"DEBUG: gender_str = {gender_str}")
    body_model = smplx.create(
        model_path=smplx_body_model_path,
        model_type="smplx",
        gender=gender_str,
        use_pca=False,
    )
