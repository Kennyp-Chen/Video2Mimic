...
class RobotMotionViewer:
    ...
    def step(self, 
        ...
        # Check if model has free joint (7+ elements) or is fixed base (only joint angles)
        if self.model.nq > len(dof_pos):
            # Model has free joint
            self.data.qpos[:3] = root_pos
            self.data.qpos[3:7] = root_rot # quat need to be scalar first! for mujoco
            self.data.qpos[7:] = dof_pos
        else:
            # Fixed base robot - only joint angles
